...
function AppointmentForm(props) {
  ...
  const validateAppointment = ({ date, hours, minutes, description }) => {
    ...
    if (!description) errors.description = "Debe ingresar una descripción.";
      ...
      errors.description = "La descripción debe tener al menos 4 caracteres.";
    else if (description.length > 25)
      errors.description =
        "La descripción no puede tener más de 25 caracteres.";
    return errors;
  };
  ...
  const handleSubmit = (event) => {
    ...
    axios
      ...
      .then(({ data }) => {
        alert(
          `Creado el turno ${data.description} el dia ${data.date} a las ${data.time}`
        );
