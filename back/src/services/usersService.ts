...
export const registerUserService = async (
  ...
  try {
    ...
    const validateMail = await UserRepository.findOneBy({
      email: createUserDto.email,
    });
    if (validateMail) throw Error("ya existe un usuario con ese E-mail");

    const validateNDni = await UserRepository.findOneBy({
      nDni: createUserDto.nDni,
    });
    if (validateNDni) throw Error("ya existe un usuario con ese DNI");

    const validateUsername = await CredentialsRepository.findOneBy({
